...
class Application_Service_Evaluation {
    ...
    public function getEvaluateReports ($shipmentId) {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = $db->select()->from(array('s' => 'shipment'), array(
            's.shipment_id',
            's.shipment_code',
        ...
        $reflexive_comments = $this->getReflexiveComment($shipmentResult);
        $result = array(
    ...
    public function getReflexiveComment($shipmentResult) {
        $reflexive_comment=[];
        if (sizeof($shipmentResult) > 0) {
            ...
            foreach ($shipmentResult as $result) {
                $ref_comment=[];
